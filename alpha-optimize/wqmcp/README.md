...
- `mode`：几个一起怎么跑
  ...
  - `concurrent`：1-10 个独立模拟并行提交，每个有自己的 id，互不影响。
    - 超出账户并发上限的项返回 `RATE_LIMITED`，整体状态为 `PARTIAL`，稍后单独重提这些项即可。
...
实际发出的设置会在 `settings_used` 里回显。`simulation_mode="QUICK"` 只算核心指标，会强制关闭 visualization，这样跑出的 alpha 不能直接提交。
...
每个工具都带 MCP 注解：只读、写入、破坏性（`submit_alpha`、`cancel_simulation`、`prodmemo_manage` 等），以及是否访问外部。出错时返回 `{"error": ...}` 字典，不抛 MCP 错误，因为 `scripts/prodmemo_daily_sync.py` 等调用方依赖这种格式。
